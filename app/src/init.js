...
(function () {
  ...
  const setCreditsValue = () => {
    ...
    creditsComputed *= 365;
    animateValue("credits-total", startValue, creditsComputed, 500);
  ...
  const fieldTemplate = (data) => html`
        ...
        <div class="numeric-field"><input @input=${creditsChangeListener} type="number" data-factor="${data.credits}" .value="${data.quantity}" data-param="${data.parameter}" class="credits" name="quantity" min="0" step="100" pattern="\\d{3}\,?\\d{3}\,?\\d{3}"></div>
